    ...
    return (
        ...
        >
            ...
            <div className="px-6 py-8">
                    ...
                    {statCards.map((stat) => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className={
                                    `flex h-12 w-12 items-center justify-center rounded-3xl ${stat.accent}`
                                }>
                ...
                <div className="mt-8 rounded-[32px] border border-white/10 bg-slate-900/80 p-6 shadow-[0_24px_80px_-40px_rgba(15,23,42,0.9)]">
                    ...
                    <div className="mt-6 overflow-hidden rounded-[28px] border border-white/10">
                        <table className="min-w-full divide-y divide-slate-800 text-left text-sm">
                            <thead className="bg-slate-950/80 text-slate-400">
                                <tr>
                                    <th className="px-6 py-4 uppercase tracking-wider">ID REGISTRASI</th>
                                    {columns.map((col) => (
                                        <th key={col.key} className="px-6 py-4 uppercase tracking-wider">
                                            {col.label}
                                        </th>
                                    ))}
                                    <th className="px-6 py-4 uppercase tracking-wider">STATUS</th>
                                    <th className="px-6 py-4 uppercase tracking-wider">REGISTERED AT</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800 bg-slate-900/95">
                                {registrants.length === 0 ? (
                                    <tr>
                                        <td colSpan={columns.length + 3} className="px-6 py-12 text-center text-sm text-slate-500">
                                            No registrants yet.
                                        </td>
                                    </tr>
                                ) : (
                                    registrants.map((registrant, i) => (
                                        <tr key={i} className="border-b border-slate-800 last:border-0">
                                            ...
                                            {columns.map((col) => (
                                                <td key={col.key} className="px-6 py-5 text-slate-200">
                                                    {registrant.data[col.key]}
                                                </td>
                                            ))}
